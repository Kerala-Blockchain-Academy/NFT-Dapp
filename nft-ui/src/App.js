...
function App() {
  ...
  const getColorTokenDetails = async () => {
    const nFTCount = await NFTMarketContract.methods.getTokenCount().call();
    console.log(nFTCount);
    let colorTokenDetailsList = [];
    for (var i = 1; i <= nFTCount; i++) {
      const colorNFTValue = await NFTMarketContract.methods.colorsName(i).call();
      const ownerNFT = await NFTMarketContract.methods.ownerOf(i).call();
      let newColorToken = {
  ...
  const getItemDetails = async () => {
    ...
    for (var i = 1; i <= itemCount; i++) {
      const itemDetail = await NFTMarketContract.methods
        .idToMarketItem(i)
        .call();
      const colorNFTValue = await NFTMarketContract.methods
